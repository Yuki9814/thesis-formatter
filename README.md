...
```bash
python -m app.main format --template template.docx --content content.docx --mapping mapping.json --out output.docx --report validation_report.html [--strict] [--debug-dir debug] [--force]
```
...
- `--force`：允许覆盖已存在的普通输出 `.docx`；默认不覆盖既有输出文件。
...
- 输入文件永远不会被修改。
- 输出总是写到新的 `.docx` 路径。
- 默认拒绝覆盖已存在的输出文件；只有显式 `--force` 才会覆盖普通输出 `.docx`。
- 默认拒绝不安全 `.docx` 包结构、异常压缩比、危险 XML、宏、ActiveX、altChunk 和不安全 relationship。
- 普通外部 HTTP(S) 超链接和嵌入对象会进入安全复核项，交付前必须在 Microsoft Word 中确认是否保留。
...
```bash
uv run --python 3.11 --extra dev bandit -r app core gui models
uv run --python 3.11 --extra dev pip-audit
```
...
No formatter or type-check tool is configured yet. Add `ruff`, `mypy`, or `pyright` before documenting those commands as required checks.
